  ...
  return (
    <div className="space-y-8 text-left">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-1 bg-slate-900 border border-[#1e293b] rounded-xl p-5 shadow-lg space-y-4">
          <h2 className="text-sm font-bold pb-2 border-b border-[#1e293b] flex items-center gap-2 text-white">
          ...
          <div className="space-y-4 text-xs text-slate-400">
            <div className="flex justify-between items-center">
              ...
              <span className="font-extrabold text-white">{stats.totalCategories || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span>Tags</span>
              <span className="font-extrabold text-white">{stats.totalTags || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span>Library Attachments</span>
              <span className="font-extrabold text-white">{stats.mediaFilesCount || 0}</span>
            </div>
            <div className="flex justify-between items-center">
              <span>Storage Size</span>
              <span className="font-extrabold text-white">{(stats.storageUsage / (1024 * 1024)).toFixed(2)} MB</span>
            </div>
          </div>
        </div>

        {/* Right: Security Audit log preview */}
        <div className="lg:col-span-2 bg-slate-900 border border-[#1e293b] rounded-xl p-5 shadow-lg space-y-4 flex flex-col justify-between h-[300px] lg:h-auto">
          <h2 className="text-sm font-bold pb-2 border-b border-[#1e293b] flex items-center justify-between text-white">
          ...
          <div className="flex-1 overflow-y-auto space-y-3.5 pr-1 pt-2">
            {logs.length === 0 ? (
              ...
              logs.map((log) => (
                <div key={log._id} className="p-3 bg-slate-950 border border-[#1e293b] rounded-lg flex items-center justify-between gap-4 text-xs">
                  <div>
                    <p className="font-bold text-white capitalize">{log.action.replace(/_/g, ' ')}</p>
                    <p className="text-[10px] text-slate-400 mt-0.5">By: {log.userId?.name || 'System'} | IP: {log.ip || 'Local'}</p>
                  </div>
      ...
      <div className="bg-slate-900 border border-[#1e293b] rounded-xl p-6 shadow-lg text-center space-y-4 max-w-xl mx-auto">
        <CheckCircle className="h-10 w-10 text-[#ef4444] mx-auto" />
        <h3 className="text-base font-bold text-white">Interactive Review Queue</h3>
        <p className="text-xs text-slate-400 max-w-md mx-auto">Access the review moderation console to approve or request revisions for contributed content.</p>
