...
export default function AdminSettingsPage() {
  ...
  return (
    <div className="space-y-6 text-left max-w-xl mx-auto">
      ...
      <form onSubmit={handleSave} className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm space-y-6 text-slate-800">
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="font-bold text-xs text-slate-800">Maintenance mode</p>
            <p className="text-[10px] text-slate-500 mt-0.5">Restrict access to admin users only and show a maintenance splash page.</p>
          </div>
          <input
            type="checkbox"
            checked={maintenanceMode}
            onChange={(e) => setMaintenanceMode(e.target.checked)}
            className="w-8 h-4 bg-white border border-slate-300 checked:bg-[#ef4444] rounded-full cursor-pointer focus:outline-none"
          />
        </div>
        ...
        <div className="flex items-center justify-between gap-4">
          <div>
            <p className="font-bold text-xs text-slate-800">Allow Public Registrations</p>
            <p className="text-[10px] text-slate-500 mt-0.5">Enable public signup pages and allow verify flows.</p>
          </div>
          <input
            type="checkbox"
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Session Timeout (hours)</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-[10px] font-bold uppercase text-slate-500 mb-1">Rate Limit (req/min)</label>
            <input
              type="number"
              value={rateLimit}
