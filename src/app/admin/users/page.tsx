    ...
    <div className="space-y-6 text-left">
      ...
      {loading ? (
        ...
        <AdminTable headers={['User', 'Email', 'Role', 'Status', 'Joined Date', 'Actions']}>
          {users.map((u) => (
            <tr key={u._id} className="hover:bg-slate-50 transition-all border-b border-slate-100">
              <td className="p-4 font-semibold flex items-center gap-3">
                <div className="w-8 h-8 rounded-full overflow-hidden bg-slate-100 border border-slate-200 flex items-center justify-center text-xs font-bold text-[#ef4444]">
                  {u.avatar ? (
              ...
              <td className="p-4 capitalize text-xs">
                <span className="bg-slate-100 border border-slate-200 px-2 py-0.5 rounded font-semibold text-[10px] text-slate-600">{u.role}</span>
              </td>
              <td className="p-4">
              ...
              <td className="p-4 text-right space-x-1.5 whitespace-nowrap">
                  ...
                  <>
                    ...
                    <button
                      ...
                      className={`p-1.5 rounded bg-transparent border-0 cursor-pointer text-xs ${
                        u.status === 'active' ? 'text-yellow-600 hover:bg-yellow-500/10' : 'text-green-600 hover:bg-green-500/10'
                      }`}
                ...
                <button
                  ...
                  className="p-1.5 text-blue-500 hover:bg-blue-500/10 rounded bg-transparent border-0 cursor-pointer text-xs"
                ...
                {u.role !== 'admin' && u.role !== 'super_admin' && (
                  <button
                    ...
                    className="p-1.5 text-red-500 hover:bg-red-500/10 rounded bg-transparent border-0 cursor-pointer text-xs"
                    title="Delete User"
                  >
      ...
      {showRoleModal && selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-white border border-slate-200 rounded-xl p-6 w-full max-w-sm space-y-4 shadow-xl animate-in fade-in zoom-in-95 duration-150 text-left text-slate-800">
            <h3 className="font-bold text-sm text-slate-800">Change Account Role</h3>
            <p className="text-xs text-slate-500">Modify authorization level for <strong>{selectedUser.name}</strong>.</p>
            <select
              value={newRole}
            ...
            <div className="flex justify-end gap-2 text-xs font-bold pt-2">
              <button onClick={() => setShowRoleModal(false)} className="px-3.5 py-2 border border-slate-200 rounded-lg bg-transparent text-slate-500 hover:bg-slate-50 cursor-pointer">
                Cancel
              </button>
      ...
      {showPasswordModal && selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
          <div className="bg-white border border-slate-200 rounded-xl p-6 w-full max-w-sm space-y-4 shadow-xl animate-in fade-in zoom-in-95 duration-150 text-left text-slate-800">
            <h3 className="font-bold text-sm text-slate-800">Override User Password</h3>
            <p className="text-xs text-slate-500">Apply a new secure password credential for <strong>{selectedUser.name}</strong>.</p>
            <input
              type="password"
              value={newPassword}
            ...
            />
            <div className="flex justify-end gap-2 text-xs font-bold pt-2">
              <button onClick={() => setShowPasswordModal(false)} className="px-3.5 py-2 border border-slate-200 rounded-lg bg-transparent text-slate-500 hover:bg-slate-50 cursor-pointer">
                Cancel
              </button>
